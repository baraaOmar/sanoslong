...
$con=con();
  ...
  //get user id
   ...
   $checkEnd="select date_start from p_sub where   DATE_ADD('$start',INTERVAL $period month)>=DATE_ADD('$date',INTERVAL '$i' day) and  approved=1 and user_id= ".$id;
   if(empty($result=$con->query($checkEnd))){
     echo "no supscription diet in this date";
   }
   else{
